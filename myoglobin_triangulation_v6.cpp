...
//	generate_packed_sphere();
	...
	Vertex_handle target_vertex = target_ch->vertex(0);
	
	vector<Cell_handle> all_target_cells;
	//vector<Cell_handle> > target_cell_inserter(all_target_cells);
	...
	cout<<"Number of target cells "<<all_target_cells.size()<<endl;
